    ...
    for (let tok of tokens) {
        ...
        if (bracket == tok)
            return closed == tok ? handleSame(state, tok, tokens.indexOf(tok + tok + tok) > -1)
                : handleOpen(state, tok, closed, conf.before || defaults.before, conf.notAfter || defaults.notAfter)
...
function handleOpen(state: EditorState, open: string, close: string, closeBefore: string, doNotCloseAfter: string) {
    let dont = null, changes = state.changeByRange(range => {
        if (!range.empty)
            return {
        ...
        let prev = prevChar(state.doc, range.head)
        if ((!next || /\s/.test(next) || closeBefore.indexOf(next) > -1) &&
            (prev == "" || doNotCloseAfter.indexOf(prev) < 0))
            return {
