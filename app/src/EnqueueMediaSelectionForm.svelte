<script lang="ts">
    ...
    export let mediaKind: MediaSelectionKind = "video";
    ...
    <div class="mt-4 space-y-4">
        <div class="flex items-start">
            ...
            <div class="ml-3 text-sm">
                ...
                <p class="text-gray-500">Staff-only option that will make this entry appear as enqueued by JungleTV.</p>
            </div>
        </div>
    </div>
{/if}
    ...
    <div class="flex items-start">
        ...
        <div class="ml-3 text-sm w-full">
            {#if videoIsBroadcast}
                ...
                <p class="text-gray-500">
                    Broadcasts can play for up to {maxRangeLength / 60} minutes at a time and up to a total of 2 hours in
                    the last 4 hours. Prices will be relative to the length that plays.
                </p>
            {:else}
                <label for="videorange" class="font-semibold text-gray-700 dark:text-gray-300">
                ...
                <p class="text-gray-500">
                    ...
                    {#if mediaLengthInSeconds > maxRangeLength}
                        <br />
                        {mediaKind == "video" ? "Videos" : "Tracks"} longer than {maxRangeLength / 60} minutes must be enqueued
                        in shorter non-overlapping segments, each up to {maxRangeLength / 60} minutes long.
                    {/if}
                </p>
            {/if}
                ...
                {#if mediaRangeValuesFilled}
                    ...
                    {#if mediaLengthInSeconds <= defaultMinRangeLength}
                        <p class="text-red-500">
                            This {mediaKind} is shorter than {defaultMinRangeLength} seconds and can only be enqueued in
                            its entirety.
                        </p>
                    {/if}
