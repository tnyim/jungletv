...
// taken from https://github.com/brave/leo/blob/397f28fc6660b7fcb8a6b797a5454d8b22d13350/src/components/svelte-web.ts
// modified slightly to appease type checking and to allow the created shadow roots to be manipulated by callbacks,
// and also fixed a bug in attributeChangedCallback where attributePropMap was not being used
...
interface Options {
    mode: 'open' | 'closed'
    name: string,
    shadowRootPreparer?: (shadow: ShadowRoot) => void,
    childrenUpdatedCallback?: (children: HTMLCollection) => void,
}
 ...
 */
const createSlot = (name?: string | null) => {
...
export default function registerWebComponent(
    component: any,
    { name, mode, shadowRootPreparer, childrenUpdatedCallback }: Options
) {
    ...
    class SvelteWrapper extends HTMLElement {
        ...
        constructor() {
            ...
            if (shadowRootPreparer) {
                shadowRootPreparer(shadow);
            }
        ...
        attributeChangedCallback(name, oldValue, newValue) {
            ...
            this[attributePropMap.get(name) ?? name] = newValue
        }
